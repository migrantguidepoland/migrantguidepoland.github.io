...
let currentLang = 'ua'; // Default to Ukrainian since it's selected in the UI
...
function performFAQSearch(query) {
    const responses = {
        pl: `Szukam odpowiedzi na pytanie: "${query}". Nasze AI analizuje zapytanie...`,
        ua: `Шукаю відповідь на питання: "${query}". Наш AI аналізує запитання...`,
        en: `Searching for answer to: "${query}". Our AI is analyzing the query...`,
        ru: `Ищу ответ на вопрос: "${query}". Наш AI анализирует запрос...`
    };

    alert(responses[currentLang]);
}

// Chatbot functionality
function initializeChatbot() {
    const chatButton = document.querySelector('.chat-launch');

    if (chatButton) {
        chatButton.addEventListener('click', function() {
            const messages = {
                pl: 'Witaj! Jestem AI asystentem MigrantGuidePoland. Jak mogę Ci pomóc?',
                ua: 'Привіт! Я AI асистент MigrantGuidePoland. Як можу допомогти?',
                en: 'Hello! I am AI assistant of MigrantGuidePoland. How can I help you?',
                ru: 'Привет! Я AI ассистент MigrantGuidePoland. Как могу помочь?'
            };
